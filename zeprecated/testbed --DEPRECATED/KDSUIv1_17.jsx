...
// Create dialog
var dialog = new Window("dialog", "KDSUI v1.03a");
...
modifyPanel.margins = 16;
        ...
        widthInput.onChanging = function () {
            if (widthInput.text !== "") {
                ...
                dontModifyWidthCheckbox.value = false;
            }
        }
        // Add checkbox for skipping text width
        var widthCheckbox = widthGroup.add("checkbox", undefined, "Text Frame Width");
        ...
        // Add checkbox for not modifying width
        var dontModifyWidthCheckbox = widthGroup.add("checkbox", undefined, "Don't Modify Width");
        dontModifyWidthCheckbox.value = false;
        dontModifyWidthCheckbox.onClick = function () {
            if (dontModifyWidthCheckbox.value) {
                widthInput.text = "";
                widthCheckbox.value = false;
            }
        }
...
startButton.onClick = function () {
    ...
    var width = dontModifyWidthCheckbox.value ? "skip" : (widthCheckbox.value ? null : widthInput.text);
    runScript(fratNames, fratOutputPath, heightInput.text, actionInput.text, width, skipActionCheckbox.value, skipHeightCheckbox.value);
    resetState();
    runScript(soroNames, soroOutputPath, heightInput.text, actionInput.text, width, skipActionCheckbox.value, skipHeightCheckbox.value);
    dialog.close();
}
...
function runScript(names, output, height, action, width, skipAction, skipHeight) {
    ...
    for (var i = 0; i < names.length; i++) {
        ...
        // Store the initial geometric bounds of the text frame
        var initialBounds = textFrame.geometricBounds;

        // Replace the text in the text frame
        textFrame.contents = names[i];

        // Only modify the width if the "Don't Modify Width" checkbox is not checked
        if (width !== "skip") {
        ...
        // Set the geometric bounds of the text frame back to its initial bounds
        textFrame.geometricBounds = initialBounds;
