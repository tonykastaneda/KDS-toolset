...
def main():
    ...
    with tqdm(total=len(files), ncols=70) as pbar:
        for file in files:
            ...
            client_id = get_client_id(image_name)
            if client_id is None:  # Skip this image if no matching client is found
                continue
    ...
    with open('debug.txt', 'w') as debug_file:
        for data in data_list:
            debug_file.write(json.dumps(data, indent=4))
            debug_file.write('\n')



    response = requests.post('https://api.stage.affinitygateway.com/designs', headers=headers, data=json.dumps(data_list))
