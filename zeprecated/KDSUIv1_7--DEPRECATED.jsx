...
// Create main window
var win = new Window("dialog", "KDSUI 1.6");
...
function runScript(names, outputLocation, height, action, width, fontSizeFirst, fontSizeLast, originalFileLocation) {
    for (var i = 0; i < names.length; i++) {
        ...
        // Change the font size of the last character
        if (textFrame.characters.length > 1) {
            var fontSizeLastNum = parseFloat(fontSizeLast);
            if (!isNaN(fontSizeLastNum)) {
                textFrame.characters[textFrame.characters.length - 1].characterAttributes.size = fontSizeLastNum;
            }
        }
