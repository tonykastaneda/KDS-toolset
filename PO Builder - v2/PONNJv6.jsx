function replaceAll(str, find, replace) {
    return str.split(find).join(replace);
}

// Get the directory where the script is located
var scriptFolder = new File($.fileName).parent; 

// Get all .ai files in the script's directory
var files = scriptFolder.getFiles("*.ai");

// Create a log file in the same folder as the script
var logFile = new File(scriptFolder.fsName + "/log.txt");
logFile.open("w");

for (var i = 0; i < files.length; i++) {
    var file = files[i];
    var parts = file.name.split('-');
    var extractedName = decodeURI(parts[1]);
    extractedName = replaceAll(extractedName, "%20", " ");
    var number = parts[2].split('-')[0];

    // Write the extracted name and number to the log file
    logFile.writeln("File: " + file.name);
    logFile.writeln("Extracted Name: " + extractedName);
    logFile.writeln("Extracted Number: " + number);

    var doc = app.open(file);
    var namTextFrame = doc.textFrames.getByName("nam");

    // Only set the contents of the "nam" text frame if extractedName is not 'NN'
    if (extractedName !== "NN") {
        namTextFrame.contents = extractedName;

        /* -------------------- Only effect names longer than # bellow  ------------------- */
        if(extractedName.length > 7) {
            /* ----------------------- Set the font size to # above ----------------------- */
            namTextFrame.textRange.characterAttributes.size = 185;
            /* ------------ Move the text frame up (36 points in half an inch) ------------ */
            // namTextFrame.position = [namTextFrame.position[0], namTextFrame.position[1] + 36];
        }
    }

    // Loop over all text frames
    for (var j = 0; j < doc.textFrames.length; j++) {
        var textFrame = doc.textFrames[j];

        // Check if the text frame is named "numf" or "numb"
        if ((textFrame.name === "numf" || textFrame.name === "numb") && number !== "NN") {
            textFrame.contents = number;
        }
    }

    doc.close(SaveOptions.SAVECHANGES);
}

// Close the log file
logFile.close();